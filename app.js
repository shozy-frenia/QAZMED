...
/* ===== live PK simulator (RK4 of the team's ODEs) ===== */
...
function auc(cl){ return P.Dose*56/cl; }   // simplified AUC (mg·h/L), per model
function phenoConv(il6){ return Math.min(0.15, 0.033*il6); } // IL-6 -> f_pheno
...
function remission(cl, age, pgx){
  const A = auc(cl)/1000;                    // β1 defined per 1000 mg·h/L
...
async function ask(text){
  addMsg(text,'u'); history.push({role:'user',content:text});
  const t=typing(); sendBtn.disabled=true;
  try{
    const res=await fetch('https://api.anthropic.com/v1/messages',{
      method:'POST', headers:{'Content-Type':'application/json'},
      body:JSON.stringify({ model:'claude-sonnet-4-6', max_tokens:600, system:SYS, messages:history })
    });
    if(!res.ok) throw new Error('http '+res.status);
    const data=await res.json();
    const txt=(data.content||[]).filter(b=>b.type==='text').map(b=>b.text).join('\n').trim();
    t.remove();
    if(txt){ addMsg(txt,'a'); history.push({role:'assistant',content:txt}); }
    else throw new Error('empty');
  }catch(err){
    t.remove();
    const fb=kbLookup(text);
    if(fb){ addMsg(fb,'a'); }
    else{ addMsg("I can't reach the live AI service from here, so I'm limited to the built-in topics. Try one of the suggested questions, or run this page inside Claude / behind a small backend proxy to enable full answers.\n\n_Educational only — not medical advice._",'a'); }
...
/* respect reduced motion for the hero particle */
